...
twoDaccelviewer::twoDaccelviewer(bool livedisplaymode, QString titlename, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::twoDaccelviewer)
{
    ...
    reset();
    //Mainscene->clear();
    myQSL = titlename.split(QRegExp("[_\\.]"));
    mypdn = 255;
    if(myQSL.size() && !QString::compare(myQSL.last(), QString("edf")))
    {
        mypdn = myQSL.at(myQSL.size()-2).toUInt();
    }
                              ...
                              qint64 firstts, qint64 lastts)
{
    ...
    mylastFrmTS=lastts;
    firstFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs(myfirstFrmTS*1000/UCERTCFREQ);
    // align to full second
    firstFrmDT.setMSecsSinceEpoch(((firstFrmDT.toMSecsSinceEpoch()+999)/1000)*1000);
    lastFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs(mylastFrmTS*1000/UCERTCFREQ);
    // align to full second
    ...
    for(i =0 ; i<mytsp->size();i++)
    {
        if((mytsp->at(i)>=myfirstFrmTS)&&(mytsp->at(i)<=mylastFrmTS))
        {
            ...
            for(j=0;j<ACCLEN;j++)
            {
                if((i*ACCLEN+j)<acczQVp->size())
                {
                    zzz=acczQVp->at(i*ACCLEN+j);
                    acczqvectoriginal.append(acczQVp->at(i*ACCLEN+j));
                    if(zzz>32767) zzz-=32768;
                    z_new_data_average+=zzz/ACCLEN; //prevent overflow in qint16
                }
            }
        ...
        for(i = 0; i<sizemyts/reduction;i++) //reduction
        {
            if((mytsQV.at(i*reduction)>=myfirstFrmTS)&&(mytsQV.at(i*reduction)<=mylastFrmTS)) //reduction
            {
                if(firstdraw)
                {
                    ...
                    oldposx = posx;
                    oldposy = posy;
                    start=Mainscene->addText("start", QFont("Arial", 9));
            ...
            if(mytsQV.at(i*reduction)>=mylastFrmTS) //reduction
            {
                if(!firstdraw)
                {
                    calnewpos(&posx,&posy,accxqvect.at(i*reduction),accyqvect.at(i*reduction),acczqvect.at(i*reduction));//reduction
                    draw(posx,posy,&oldposx,&oldposy);
                    end=Mainscene->addText("end", QFont("Arial", 9));
...
void twoDaccelviewer::showActivityText()
{
    ...
    if(totalcount!=0)
    {
        tempQS.append(QString("Activity Time: %1s\n").arg(totalcount/FRMSPERSEC));
        tempQS.append(QString("Pedometer: %1 steps\n").arg(stepcount));
        tempQS.append(QString("LowAct:  %1s %2\%\n").arg(lowcount/FRMSPERSEC).arg(lowcount*100/totalcount));
        tempQS.append(QString("MedAct:  %1s %2\%\n").arg(medcount/FRMSPERSEC).arg(medcount*100/totalcount));
        tempQS.append(QString("HighAct: %1s %2\%\n").arg(highcount/FRMSPERSEC).arg(highcount*100/totalcount));
        tempQS.append(QString("Intense: %1s %2\%").arg(intensecount/FRMSPERSEC).arg(intensecount*100/totalcount));
    }

    // show the activity text
    if(activityText == 0)
    {
        activityText = Mainscene->addText(tempQS, QFont("Arial", 9));
    }
    else
    {
...
void twoDaccelviewer::showMetaAnalysisText(QString *sleepQS)
{
    if(metaAnalysisText == 0)
        metaAnalysisText = Mainscene->addText(*sleepQS, QFont("Arial", 9));
    else
        metaAnalysisText->setPlainText(*sleepQS);
    metaAnalysisText->setPos(10,this->height()/2);
}
...
void twoDaccelviewer::on_genActTextPB_clicked()
{
    ...
    outfileName.append(QString("E%1_%2_act.txt").arg(firstFrmDT.toString("yyyyMMdd_hhmmss")).arg(mypdn));
    outfileTp = new QFile(outfileName);
    if(outfileTp->exists())
    {
        msgBox.setWindowTitle("Output Text File Exists");
        msgBox.setText(QString("Out file %1 already exists in selected directory.").arg(QString("E%1_%2_act.txt").arg(firstFrmDT.toString("yyyyMMdd_hhmmss")).arg(mypdn)));
    ...
    *outTstrp << QString("Activity Time: %1 sec\n").arg(totalcount/FRMSPERSEC);//
    if(totalcount!=0)
    {
        *outTstrp << QString("Pedometer: %1 steps\n").arg(stepcount);
        *outTstrp << QString("LowAct:  %1 sec %2\%\n").arg(lowcount/FRMSPERSEC).arg(lowcount*100/totalcount);
        *outTstrp << QString("MedAct:  %1 sec %2\%\n").arg(medcount/FRMSPERSEC).arg(medcount*100/totalcount);
        *outTstrp << QString("HighAct: %1 sec %2\%\n").arg(highcount/FRMSPERSEC).arg(highcount*100/totalcount);
        *outTstrp << QString("Intense: %1 sec %2\%").arg(intensecount/FRMSPERSEC).arg(intensecount*100/totalcount);
    }
